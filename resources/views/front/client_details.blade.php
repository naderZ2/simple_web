...
<section class="map-back">
      <div class="container">
        ...
        <li> <a href="{{route('clients',app()->getLocale())}}">{{ __('s_clients') }}</a></li>
        <li>{{app()->getLocale() == "en" ? $result_client->name_en : $result_client->name}}</li>
        </ul>
      </div>
    </section><!-- End Section panner Top -->

    <section class="happy-clients inner-bac">
      <div class="container">
        <div class="row">
          <div class="col-sm-3 innerclient">
            <div class="innerphoto"><img src="{{url('uploads/clients/'.$result_client->picture)}}" alt='{{app()->getLocale() == "en" ? $result_client->name_en : $result_client->name}}' title='{{app()->getLocale() == "en" ? $result_client->name_en : $result_client->name}}'></div>
          </div>
          <div class="col-sm-9 innerclient">
            <div class="destext">
              <h2 class="title">{{app()->getLocale() == "en" ? $result_client->name_en : $result_client->name}}</h2>
              <p class="text">

              {!! app()->getLocale() == "en" ? strip_tags($result_client->details_en) : strip_tags($result_client->details) !!}              
              </p>
            </div>
            <div class="destext">
              <div class="map-text"> 
                ...
                <p>{{app()->getLocale() == "en" ? $result_client->address_en : $result_client->address}}</p>
              </div>
              <div class="map-text"> 
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div><a href="tel:{{$result_client->phone}}"> {{$result_client->phone}}  </a></div>
              </div>
              <div class="map-text"> 
    ...
    <section class="clients">
      <div class="container">
        ...
        <!--<p class="text-center">{{ __('s_happy_clients_details')}}</p>-->
        <div class="clients-slider">
          @foreach($result_relatet_clients as $result_relatet_client)
            <div class="client-item">
                <a class="photo" href="{{route('client_details',[app()->getLocale(), $result_relatet_client->id])}}" title='{{app()->getLocale() == "en" ? $result_relatet_client->name_en : $result_relatet_client->name}}'> 
                    <img src="{{url('uploads/clients/'.$result_relatet_client->picture)}}" alt='{{app()->getLocale() == "en" ? $result_relatet_client->name_en : $result_relatet_client->name}}' title='{{app()->getLocale() == "en" ? $result_relatet_client->name_en : $result_relatet_client->name}}'>
                </a>
                <div class="title-it">{{app()->getLocale() == "en" ? $result_relatet_client->name_en : $result_relatet_client->name}}</div>
                <p class="text">
                {!! app()->getLocale() == "en" ? strip_tags(Str::words($result_relatet_client->details_en, $words = 30, $end = '')) : strip_tags(Str::words($result_relatet_client->details, $words = 30, $end = '')) !!}
                </p>
                <a class="bottom" href="{{route('client_details',[app()->getLocale(), $result_relatet_client->id])}}" title='{{app()->getLocale() == "en" ? $result_relatet_client->name_en : $result_relatet_client->name}}'><i class="fas fa-chevron-right"></i>{{ __('s_more')}}</a>
            </div>
          @endforeach
        </div>
      </div>
